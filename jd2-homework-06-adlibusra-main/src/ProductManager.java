...
public class ProductManager {
  ...
  public List<Product> getActiveProductsSortedByPrice() {
	  ...
	  List<Product> activeProduct= products.values() .stream().filter(p-> p.getProductStatus().equals(ProductStatus.ACTIVE)).sorted(Comparator.comparingDouble(Product :: getPrice)).toList(); 
  ...
  public double calculateAveragePriceInCategory(String category) {
    ...
    // tip: OptionalDouble kullanımını inceleyin.
	  OptionalDouble productAvarage= products.values()
.stream().filter(p ->p.getCategory().equals(category)).mapToDouble(Product :: getPrice ).average() ;
	  return productAvarage.orElse(0.0) ;
  }
  ...
  public Map<String, Double> getCategoryPriceSum() {
    ...
    // category-2 45.0
	  ...
	  Map<String, Double> totalAmountPrpdcut = products.values().stream()
			  .collect(Collectors.groupingBy(Product :: getCategory,
					  Collectors.summingDouble((product) -> product.getPrice()* product.getStock())))  ;
			  
			  return totalAmountPrpdcut;
  }
}
